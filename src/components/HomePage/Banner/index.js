...
export default function Banner({ datas, propsData }) {
    ...
    return (
        <div className="w-full -mt-1 mx-auto bg-banner -z-10 flex gap-8 lg:gap-24 items-center p-4 h-80" >
            <div className="text-white font-bold w-full md:w-1/2 p-4 h-full relative">
                ...
                <form className="w-full rounded-full shadow-lg text-red-900 my-4 flex relative lg:mt-20 mt-10" >
                    <input className="w-full font-normal rounded-full p-2 px-4 border-0" type="search" placeholder={propsData.placeholder} />
            ...
            <div className="group hidden md:flex items-center text-white w-1/2 gap-2">
                <div className="opacity-0 group-hover:opacity-100 transition-all ease-in-out duration-300 cursor-pointer hover:scale-105" onClick={handlerPrevImage}>
                ...
                </div>
                    {
                        datas.map((e, i) => {
                            if(i === index) {
                                return <Link key={e.id} className="hover:scale-105 transition-all ease-in-out duration-300 shadow-lg" href={"#"} >
